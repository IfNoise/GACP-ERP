...
export class SpatialRepository {
  ...
  /**
   * Find zone with all its sub-zones (flattened list).
   */
  ...
  /**
   * Find zone hierarchy recursively (parent → sub-zones → racks).
   * Returns a tree structure with nested sub_zones and racks.
   */
  async findZoneHierarchy(zoneId: string): Promise<ZoneWithHierarchy | null> {
    const zone = await this.findZoneById(zoneId);
    if (!zone) return null;

    // Find direct sub-zones
    ...
    const subZones = await Promise.all(
      subZoneRows.map(async (subZoneRow) => {
        ...
        // Recursively get racks for this sub-zone
        const racks = await this.findRacksByZone(subZoneRow.id);
        return { ...mapped, racks };
      }),
    );

    // Find racks directly in this zone (not in sub-zones)
  ...
  /**
   * Find all racks in a zone with their shelves and trays.
   */
  async findRacksByZone(zoneId: string): Promise<RackWithShelves[]> {
    ...
    return Promise.all(
      racks.map(async (rack) => {
        ...
        return { ...rack, shelves: shelvesWithTrays };
      }),
    );
  }

  /**
   * Find top-level zones (no parent).
   */
  ...
  /**
   * Auto-generate next rack code for a zone: R-001, R-002, …
   */
